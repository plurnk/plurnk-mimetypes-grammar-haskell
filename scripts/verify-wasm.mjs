#!/usr/bin/env node
// CI byte-identical reproducibility check for haskell.wasm.
...
const committed = await readFile(path.join(repoRoot, "haskell.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed haskell.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-haskell-verify-"));
...
console.log(`rebuilt haskell.wasm sha256: ${rebuiltHash}`);
