#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-haskell.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-haskell-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-haskell.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as haskell.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "haskell.wasm"));
const bytes = (await readFile(path.join(repoRoot, "haskell.wasm"))).length;
console.log(`haskell.wasm: ${bytes} bytes (built from ${pin})`);
